...
combined_metadata_dat <- combined_metadata_dat %>%
  rename(
    sample_no = name,   # Rename "name" to "sample_no"
    mouse_no = sample_no # Rename "sample_no" to "mouse_no"
  )
data_path <- "data/metadata/"

saveRDS(combined_metadata_dat, file = paste0(data_path, "combined_metadata_dat", ".rds"))
